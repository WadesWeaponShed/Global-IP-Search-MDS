...
mgmt_cli -r true show mdss details-level full --format json | jq -r --arg IP "$IP" '.objects[]."global-domains"[] | "mgmt_cli -r true -d " + .name + " show objects ip-only true filter " + $IP + " --format json" ' >> search.sh
chmod +x search.sh
/bin/bash search.sh
rm search.sh
