...
mgmt_cli -r true show mdss details-level full --format json | jq -r --arg IP "$IP" '.objects[]."global-domains"[] | "mgmt_cli -r true -d " + .name + " show objects filter " + $IP + " limit 500 --format json" ' >> search.sh
chmod +x search.sh
